...
function Main() {
  ...
  users.forEach((user, index) => {
    ...
    str += `<div class="box">
        <div class="time">
          ${user.available ? `<div class="avi">available</div>` : ""}
          <div class="hr">${user.rate}/hr</div>
        </div>
        <div class="img">
          <img
        ...
        <div class="text">
          ...
          <h2 class="profession">${user.designation}</h2>
          <h3 class="work"><i class="ri-building-line"></i> ${user.company}</h3>
        </div>
        <div class="skill">
         <div class="ski">${user.skills[0]}</div>
         <div class="ski">${user.skills[1]}</div>
         <div class="ski">${user.skills[2]}</div>
         <div class="ski skil">+${user.skills.length - 3}</div>
        </div>
        <div class="des">
          ${user.description}
        </div>
        ...
        <div class="${user.status}" id=${index}>${
      user.status == "Stranger" ? "Add Friend" : "Unfriend"
    }</div>
      </div>`;
  });
...
boxes.addEventListener("click", (event) => {
  ...
  if (users[event.target.id].status === "Stranger") {
    ...
    users[event.target.id].isFriend = true;
  } else {
    users[event.target.id].status = "Stranger";
    users[event.target.id].isFriend = false;
  }
  Main();
});
